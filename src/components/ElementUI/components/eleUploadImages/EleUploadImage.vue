...
export default {
  ...
  watch: {
    ...
    fileList: { // 【扩展】 用于手动预览
      handler(nv) {
        if (nv.length) {
          if (this.multiple) {
            this.$emit('input', [...nv.map(v => v.url)])
          } else {
            this.$emit('input', nv[0].url)
          }
        } else {
          this.$emit('input', null)
        }
        this.$emit('update:uploadFiles', nv.filter(v => !!v.raw))
      },
      immediate: true,
      deep: true
    }
  ...
  methods: {
    ...
    async handleRemove(index) {
      const toDel = await this.confirmDelete()

      if (toDel) {
        if (!this.fileList[index].raw) { // 删除服务器图片：调用删除接口
          console.log('删除的图片是已上传的图片')
          // 删除成功
          if (this.responseDeleteFn) {
            const resDel = await this.responseDeleteFn(this.fileList[index]).then(() => true).catch(() => false)
            if (resDel) {
